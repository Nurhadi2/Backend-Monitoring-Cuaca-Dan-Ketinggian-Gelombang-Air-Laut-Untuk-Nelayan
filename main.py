...
ECHO = 14
anemometer_pin = 22
pin_interrupt = 27  # Rain gauge pin
...
# Rain gauge interrupt function
def hitung_curah_hujan(channel):
    global flag
    flag = True
    print(f"Rain gauge triggered at {datetime.now()}, flag set to True")

def count_pulse(channel):
    global count
    count += 1

# Add event detect for the rain gauge interrupt
...
def calculate_wind_speed(count, interval):
    ...
    keliling = 3.146
    wind_speed_cm_per_sec = rotations_per_sec * 2 * float(keliling) * radius
    return wind_speed_cm_per_sec / 100

def generate_sensor_data():
    global count, curah_hujan, jumlah_tip, flag
    ...
    while True:
        ...
        if flag:
            ...
            jumlah_tip += 1
            flag = False  # Reset the flag after processing
            print(f"Rainfall incremented, new intensity: {curah_hujan} mm")

        #Update_curah_hujan value based on jumlah_tip
        curah_hujan = jumlah_tip * milimeter_per_tip

        # Emit sensor data if all readings are valid
        if bme280_data and ultrasonic_distance is not None:
            combined_data = {
                ...
                'rain_intensity': round(curah_hujan, 2),  # Use actual calculated value
